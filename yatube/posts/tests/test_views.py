...
class PostsViewsTest(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.post = Post.objects.create(
            pk=1, text="first text", group=cls.group, author=cls.author
        )
        Post.objects.create(
            pk=2,
            text="second text",
            group=None, author=cls.user
        )
        cls.comment = Comment.objects.create(
            post=cls.post, author=cls.user, text="test comment"
        )
    ...
    def test_pages_use_correct_template(self):
        names_templates = {
            ...
            reverse(
                "posts:group_list", kwargs={"slug": PostsViewsTest.group.slug}
            ...
            reverse(
                "posts:post_detail", kwargs={"post_id": PostsViewsTest.post.pk}
            ): "posts/post_detail.html",
            reverse("posts:post_create"): "posts/create_post.html",
            reverse(
                "posts:post_edit", kwargs={"post_id": PostsViewsTest.post.pk}
    ...
    def test_profile_context(self):
        ...
        for post in response.context["page_obj"]:
            self.assertIsInstance(post, Post)
            self.assertEqual(
                post.author.username, PostsViewsTest.author.username
            )
